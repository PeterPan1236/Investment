...
import { computeVolatilityRegime } from './market-regime-lib.mjs';
...
const VOL_WINDOW = 20;
const PERCENTILE_LOOKBACK = 252;
const STRESS_PERCENTILE = 0.85;
...
async function main() {
  ...
  const regimeMap = computeVolatilityRegime(dailyReturnsByDate, { volWindow: VOL_WINDOW, percentileLookback: PERCENTILE_LOOKBACK, stressPercentile: STRESS_PERCENTILE });
  const stressDayCount = [...regimeMap.values()].filter((r) => r.isStress).length;
  console.log(`market regime index built: ${regimeMap.size} trading days, ${stressDayCount} flagged as stress (realized vol percentile >= ${STRESS_PERCENTILE})`);
  console.log('');
  ...
  lines.push(`- 壓力regime定義：合成指數trailing ${VOL_WINDOW}日已實現波動度，相對其自身trailing ${PERCENTILE_LOOKBACK}個交易日分布落在前 ${((1-STRESS_PERCENTILE)*100).toFixed(0)}% 分位（percentile >= ${STRESS_PERCENTILE}），該日視為「異常高波動」regime（VIX式定義，非單純報酬率門檻）。`);
  ...
  const REPORT_FILE = path.join(__dirname, '..', '..', 'reports', 'regime-conditional-stop-evaluation-volregime.md');
